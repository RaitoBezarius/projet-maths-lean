...
La première de ces tactiques est \texttt{exact}. Elle permet de dire à Lean que le but recherché correspond exactement à ce que vous lui indiquez. Par exemple, si le but est  $\exists  p:P$, et que vous disposez de p de type P dans les hypothèses, alors \textit{exact p,} terminera la preuve. De même, si le but est  $\exists  q:Q$ et que vous disposez d'un élément p de type P et d'une fonction f : $P \to Q$, alors \textit{exact f(p),} terminera la preuve.\\
