...
\textbf{rw hd,} on remplace $t \times (d + b) + t$ par $t \times d + t \times b+t$ en utilisant $hd$, on obtient $ t \times d + t \times b + t = t \times succ(d) + t \times b$\\
\textbf{rw add\_right\_comm,} : on applique la commutativité de l'addition pour remplacer $t \times b + t$ par $ t + t \times b$\\
\textbf{rw $\leftarrow$  mul\_succ, }: on utilise rw $\leftarrow$ pour remplacer $t \times d + t$ (qui est le membre droit de l'égalité qui correspond au théorème mul\_succ) par $t \times succ (d)$, on obtient $t \times succ(d) + t \times b = t \times succ(d) + t \times b$\\
\textbf{refl,} \\
...
\begin{center} pour tous entiers naturels $a$ et $b$ : $(a+b)^2=a^2+b^2+2\times a\times b $\end{center}
Preuve
 ...
 \textbf{rw one\_eq\_succ\_zero,} :  on réécrit $1$ en $succ(0)$, on obtient donc $(a + b) ^ {succ (succ (0))} = a ^ {succ (succ (0)) }+ b ^ {succ (succ (0)) }+ succ (succ (0)) \times a \times b$\\
  \textbf{repeat {rw pow\_succ},} : on obtient $(a + b) ^ 0 \times (a + b) \times (a + b) = a ^ 0 \times a \times a + b ^ 0 \times b \times b + succ (succ (0)) \times a \times b$ \\
  \textbf{repeat {rw pow\_zero},} : on obtient $1 \times (a + b) \times (a + b) = 1 \times a \times a + 1 \times b \times b + succ (succ (0)) \times a \times b$\\
  \textbf{simp,} : on obtient $(a + b) \times (a + b) = a \times a + (b \times b + a \times (b \times succ (succ (0))))
$, donc simp, dans ce cas, applique le théorème one\_mul(m : mynat) : $m \times 1 = m$\\ 
  \textbf{repeat {rw mul\_succ},} : on obtient $(a + b) \times (a + b) = a \times a + (b \times b + a \times (b \times 0 + b + b))$\\
  \textbf{simp,} : on obtient $(a + b) \times (a + b) = a \times a + (b \times b + a \times (b + b))$, donc simp, dans ce cas, applique les théorèmes mul\_zero(a : mynat):$a \times 0 = 0$  et zero\_add(n : mynat):$0 + n = n$\\
  \textit{On développe (a + b) \times (a + b) :} \\
 \textbf{rw mul\_add,} \\
 \textit{On développe $(a + b) \times a$ :}\\
  \textbf{rw mul\_comm,} \\
  \textbf{rw mul\_add,} \\
  \textit{On développe $(a + b) \times b$ :} \\
 \textbf{ rw mul\_comm (a + b) b,} \\
  \textbf{rw mul\_add,} \\
 \textbf{simp,}  \textit{On met les termes du membre de gauche dans le bon ordre } \\
  \textbf{rw $\leftarrow$ add\_assoc (a \times b) (a \times b) (b \times b),} : on obtient $a \times a + (a \times b + a \times b + b \times b) = a \times a + (b \times b + a \times (b + b))$\\
  \textbf{rw add\_right\_comm,} \\
  \textbf{rw add\_comm (a \times b) (b \times b),} \\
  \textbf{rw add\_assoc (b \times b) (a \times b) (a \times b),} : on obtient $a \times a + (b \times b + (a \times b + a \times b)) = a \times a + (b \times b + a \times (b + b))$\\
  \textit{On factorise par a :}\\
  \textbf{rw $\leftarrow$ mul\_add a b b,} : on obtient $a \times a + (b \times b + a \times (b + b)) = a \times a + (b \times b + a \times (b + b))
$\\
  \textbf{refl,}
